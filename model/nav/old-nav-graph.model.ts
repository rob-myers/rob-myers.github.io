...
export class OldNavGraph extends BaseGraph<NavNode, NavNodeOpts, NavEdge, NavEdgeOpts> {
  ...
  public get json(): OldNavGraphJson {
    return {
  ...
  public static fromJson({ nodes, edges, groupedTris }: OldNavGraphJson): OldNavGraph {
    const graph = new OldNavGraph(groupedTris.map(tris => tris.map(p => redact(Poly2.fromJson(p)))));
...
export interface OldNavGraphJson {
  nodes: NavNodeOpts[];
  edges: NavEdgeJson[];
