...
import { flatten, chooseRandomItem } from './generic.model';
...
export class Poly2 {
  ...
  // private _triangulation?: [Vector2, Vector2, Vector2][];
  ...
  constructor(public points: Vector2[] = [], public holes: Vector2[][] = []) {
    this.clearCache();
  }
  ...
  /**
   * Cut outset {walls} out of union of {walls} without holes.
   */
  public static computeWalkablePolygons(
    walls: Poly2[],
    /** Points indicating which holes are unwalkable. */
    unwalkableIndicators: Vector2[]
  ): Poly2[] {
    const navPolyOutset = 1; // <-- TODO

    const allHoles = flatten(walls.map(({ holes }) => holes)).map(ring => new Poly2(ring));
    const sorted = allHoles.sort((x, y) => (x.bounds.area <= y.bounds.area ? -1 : 1));
    const unwalkableHoles = unwalkableIndicators
      .map(point => sorted.find(poly => poly.bounds.contains(point) && poly.contains(point)))
      .filter((x): x is Poly2 => Boolean(x));

    const covered = Poly2.union(walls.map(poly => poly.clone().removeHoles()));
    const outsetPolys = flatten(walls.map(poly => poly.createOutset(navPolyOutset)));
    return Poly2.cutOut(
      // outsetPolys,
      Poly2.union(unwalkableHoles.concat(outsetPolys)),
      covered
    );
  }
   ...
   */
  public fastTriangulation(): Poly2[] {
