...
- They must return either null or a virtual [DOM node](https://developer.mozilla.org/en-US/docs/Web/API/Node).
  > Intutively this return value determines an HTML fragment which should be rendered.

To clarify these statements we'll consider some code:
        `}/>
...
To see it in action, try clicking the other tabs above, or [view the CodeSandbox](https://codesandbox.io/s/rogue-markup-panzoom-yq060?file=/src/panzoom/PanZoom.jsx "@external"). The guts of the PanZoom component have been autocollapsed (see line 10).
Semantically:
...
Recall the three pillars: HTML, CSS and JavaScript (JS).
...
But of the three pillars, JS is the only programming language, and the only way to dynamically change HTML and CSS.
...
one cannot write fragments of HTML syntax inside JS without using the String data type.
React developers use an extension of JS called [JSX](https://en.wikipedia.org/wiki/JSX_(JavaScript)), naturally combining JavaScript and XML syntax.
This grammatical extension only really becomes meaningful when it is transpiled (converted) back into JavaScript:
        `}/>
        ...
        <Markdown children={`
The angled XML brackets are replaced by invocations of _React.createElement_, with various parameters.
The first corresponds to the XML tag name (more on that soon).
The second represents the attributes as a JavaScript object, using null if there are no attributes.
Finally, the subsequent parameters correspond to tag's children.
The recursive nature of XML is replaced by the recursive nature of function calls.
Then to understand what PanZoom and Grid are returning _we need to understand what React.createElement returns_.

This website actually uses Preact rather than React.
Technically speaking, _react_ and _react-dom_ are understood as aliases for [@preact/compat](https://www.npmjs.com/package/@preact/compat).
I chose Preact because it has the same API as React with a much smaller codebase,
so it is easier to understand how it works.
As a result, React.createElement produces a Preact VNode as output.
